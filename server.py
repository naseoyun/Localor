...
2. 트렌드 키워드 (지역 뉴스 기반 / 물리적 결합 절대 금지 / 배경 맥락 반영)
   - 입력으로 제공되는 지역 뉴스 키워드는 프로그램 제목, 내용, 대상, 활동 등에 문자 그대로 사용하지 마세요.
      예시: "○○대교 개통", "○○축제 개최" 등의 단어를 프로그램에 그대로 등장시키지 마세요.
   - 뉴스에서 드러나는 사건 자체가 아니라 '지역 주민의 실제 관심사·변화'를 추론하세요. 
      예시: 대교 개통 → 이동성 확대, 생활권 확장, 지역 간 교류 증가 
            관광객 증가 → 지역문화 체험, 지역 정체성 관심
            인구 감소 → 공동체 회복, 세대 간 소통
            청년 유입 → 정착 지원, 지역 네트워크 형성
   - 사건사고, 범죄, 정치적 갈등, 실존 인물, 사회적 논란 관련 키워드는 주민 관심사 추론의 근거로 사용하지 마세요. 
   - 분석한 관심사 중 강점 주제와 가장 잘 어울리는 하나의 관심사만 선택하세요. 다른 관심사와 결합하거나 확장하지 마세요.

2-1. 계절 키워드 (시의성 반영 / 위 트렌드 키워드와 다르게 취급할 것)
   - 제시된 키워드 중 '봄', '여름', '가을', '겨울'이 포함되어 있다면, 이는 다른 트렌드 단어와 달리 배경 해석 없이 **그대로** 기획에 반영하세요.
   - 해당 계절의 날씨, 방학/연휴 등 생활 주기, 계절 소재(예: 여름-물놀이/방학, 겨울-실내활동/연말)를 프로그램의 운영 시기, 실내외 여부, 소재 선정에 직접 활용해 계절감이 느껴지도록 구성하세요.
   - 계절 키워드가 없다면 이 항목은 무시하세요.
...
- concept: 기획 의도 (트렌드 단어 이면의 '관심사'와, 계절 키워드가 있다면 그 계절감을 어떻게 반영했는지 함께 서술)
